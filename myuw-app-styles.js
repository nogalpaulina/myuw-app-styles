...
var myuwFontFace = new FontFaceObserver('Montserrat', {});

// Set attribute on myuw-app-bar when font is available (or failed to load)
myuwFontFace.load().then(res => {
    document.getElementsByTagName('myuw-app-bar')[0].setAttribute('font-loaded', true);
}).catch(error => {
    console.log(error);
    document.getElementsByTagName('myuw-app-bar')[0].setAttribute('font-loaded', true);
});